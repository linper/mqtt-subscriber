...
s = m:section(NamedSection, "mqtt", "mqtt", translate("MQTT Broker"), translate("The Broker will “listen” for connections on the specified Local port."))
...
s2 = m:section(NamedSection, "mqtt", "mqtt", translate("Broker settings"), "")
...
o = s2:taboption("security", Flag, "_device_sec_files", translate("Certificate files from device"), translatef("Choose this option if you want to select certificate files from device.\
...
ca_file:depends("_device_sec_files", "1")
...
cert_file:depends("_device_sec_files", "1")
...
key_file:depends("_device_sec_files", "1")
...
local certificates_link = luci.dispatcher.build_url("admin", "system", "admin", "certificates")
o = s2:taboption("client", Flag, "_device_brg_files", translate("Certificate files from device"), translatef("Choose this option if you want to select certificate files from device.\
																										Certificate files can be generated <a class=link href=%s>%s</a>", certificates_link, translate("here")))
o:depends("use_remote_tls", "1")

bridge_ca_file = s2:taboption("client", FileUpload, "bridge_cafile", "Bridge CA File", "Upload bridge CA file")
bridge_ca_file:depends({use_remote_tls="1", _device_brg_files=""})

bridge_certfile = s2:taboption("client", FileUpload, "bridge_certfile", "Bridge CERT File", "Upload bridge CERT file")
bridge_certfile:depends({use_remote_tls="1", _device_brg_files=""})

bridge_keyfile = s2:taboption("client", FileUpload, "bridge_keyfile", "Bridge Key File", "Upload bridge Key file")
