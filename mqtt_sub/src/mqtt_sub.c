...
int get_con_conf(struct client_data *client)
{
	...
	c = uci_alloc_context();
	//getting remote port
	if ((rc = get_conf_ptr(c, &ptr, path, "mqtt_sub.mqtt_sub.remote_port", PATH_LEN)) != UCI_OK)
		goto fail;
	if (ptr.o != NULL)
		con->port = atoi(ptr.o->v.string);
	//geting remote broker address
	if ((rc = get_conf_ptr(c, &ptr, path, "mqtt_sub.mqtt_sub.remote_addr", PATH_LEN)) != UCI_OK)
		goto fail;
	if (ptr.o != NULL)
		con->host = strcpy(malloc(strlen(ptr.o->v.string)+1), ptr.o->v.string);
	//geting remote username
	if ((rc = get_conf_ptr(c, &ptr, path, "mqtt_sub.mqtt_sub.username", PATH_LEN)) != UCI_OK)
		goto fail;
	if (ptr.o != NULL)
		con->username = strcpy(malloc(strlen(ptr.o->v.string)+1), ptr.o->v.string);
	//geting remote password
	if ((rc = get_conf_ptr(c, &ptr, path, "mqtt_sub.mqtt_sub.password", PATH_LEN)) != UCI_OK)
		goto fail;
	if (ptr.o != NULL)
		con->password = strcpy(malloc(strlen(ptr.o->v.string)+1), ptr.o->v.string);
	//geting keep alive value
	if ((rc = get_conf_ptr(c, &ptr, path, "mqtt_sub.mqtt_sub.keep_alive", PATH_LEN)) != UCI_OK)
		goto fail;
	if (ptr.o != NULL)
	...
	if ((rc = get_conf_ptr(c, &ptr, path, "mqtt_sub.mqtt_sub.is_clean", PATH_LEN)) != UCI_OK)
		goto fail;
	...
	if ((rc = get_conf_ptr(c, &ptr, path, "mqtt_sub.mqtt_sub.has_will", PATH_LEN)) != UCI_OK)
		goto fail;
	...
	if ((rc = get_conf_ptr(c, &ptr, path, "mqtt_sub.mqtt_sub.use_tls", PATH_LEN)) != UCI_OK)
		goto fail;
	if (ptr.o != NULL && strcmp("1", ptr.o->v.string) == 0){
		con->use_tls = true;
//==========================================================================
...
int get_will_conf(struct uci_context* c, struct uci_ptr *ptr, struct libmosquitto_will *will)
{
	...
	if ((rc = get_conf_ptr(c, ptr, path, "mqtt_sub.mqtt_sub.will_topic", PATH_LEN)) != UCI_OK)
		goto fail;
	...
	if ((rc = get_conf_ptr(c, ptr, path, "mqtt_sub.mqtt_sub.will_message", PATH_LEN)) != UCI_OK)
		goto fail;
	if (ptr->o != NULL){
	...
	if ((rc = get_conf_ptr(c, ptr, path, "mqtt_sub.mqtt_sub.will_qos", PATH_LEN)) != UCI_OK)
		goto fail;
	if (ptr->o == NULL)
		goto fail;
	will->qos = atoi(ptr->o->v.string); //on failure qos becomes 0
	//geting will retain value
	if ((rc = get_conf_ptr(c, ptr, path, "mqtt_sub.mqtt_sub.will_retain", PATH_LEN)) != UCI_OK)
		goto fail;
	if (ptr->o != NULL && strcmp("1", ptr->o->v.string) == 0)
		will->retain = true;
...
int get_top_conf(struct client_data *client)
{
	...
	curr_topic = (struct topic_data*)calloc(client->n_tops, sizeof(struct topic_data));
...
int init_client(struct mosquitto **mosq_ptr, struct client_data *client)
{
	...
	*mosq_ptr = mosq;
	...
	if (con->has_will)
		if ((rc = mosquitto_will_set(mosq, con->will->topic, con->will->payloadlen, \
		con->will->payload, con->will->qos, con->will->retain)) != EXIT_SUCCESS)
			return SUB_GEN_ERR;
...
int connect_to_broker(struct mosquitto *mosq, struct connect_data *con)
{
	...
	if ((rc = mosquitto_connect(mosq, con->host, con->port, \
	con->keep_alive)) != MOSQ_ERR_SUCCESS){
		printf("rc: %d\n", rc);
		return SUB_GEN_ERR;
	} 
	return SUB_SUC;
...
int main(int argc, char *argv[])
{
	...
	if ((rc = connect_to_broker(mosq, client->con)) != SUB_SUC)
		goto ncon_exit;
